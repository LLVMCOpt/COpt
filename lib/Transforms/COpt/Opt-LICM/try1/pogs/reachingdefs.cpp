...
namespace reachdef
{
	...
	typedef std::vector<string> varati;	typedef std::vector<Value *> valati;	// set of definitions
	std::map<int, varati> VarAtI;		std::map<int, valati> ValAtI;			// at every instruction
	typedef std::map<int, varati> ItoS; typedef std::map<int, valati> ItoV;		
	std::map<string,ItoS> LiveAtF;		std::map<string,ItoV> RDAtF;			// for every function
		...
		virtual bool runOnFunction(Function &F)
		{
			...
			for (Function::iterator BI = F.begin(), BE = F.end(); BI != BE; ++BI)
			{
				...
				{	// iterate forward through the block, update liveness
					...
					std::vector<string> s1; std::vector<Value *> v1; // hold reaching definitions to an instruction
						...
						for (int i = 0; i < domain.size(); i++) 
							...
							{ 
								...
								if (find(s1.begin(),s1.end(),insertstr)==s1.end())
								{
									...
									v1.push_back((Value *)domain[i]);
								}
							}
				
						VarAtI.insert(std::pair<int,varati>(InstrToIndex[&*insn],s1));	// at the instruction point
						ValAtI.insert(std::pair<int,valati>(InstrToIndex[&*insn],v1));
						/*------------------------------------------------------*/
			...
			RDAtF.insert(std::pair<string,ItoV>(function_name,ValAtI));
