...
namespace llvm 
{
	...
	DataFlowResult DataFlow::run(Function &F, std::vector<void*> domain, BitVector boundaryCond, BitVector initCond)	// analyse F
	{
		...
		*intOtherVal = initCond;
		...
		std::map<BasicBlock*, BasicBlockList > blockNeighbors = std::map<BasicBlock*, BasicBlockList >();

		for (Function::iterator BB = F.begin(), BE = F.end(); BB != BE; ++BB) 
		{
			BasicBlockList neighborList = BasicBlockList();
		...
		BasicBlockList traversalOrder = BasicBlockList();
		...
		// output
		DataFlowResult output = DataFlowResult();
		output.domainToIndex = domainToIndex;
		output.result = result;
		output.modified = modified;
		errs()<<output.modified<<" run end\n";
		return output;
	}
